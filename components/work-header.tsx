...
export function WorkHeader({ onCategoryChange, title = "Our Work", showFilters = true, subtitle }: WorkHeaderProps) {
  ...
  return (
    <div className="bg-black text-white font-sans pt-25 lg:pt-10 pb-4 px-1 px-3 lg:px-8">
      <div className="max-w-full mx-auto">
        ...
        <div className="flex justify-start mb-1">
          ...
          <div className="flex-1">
            <h1
              ref={titleRef}
              className="text-[72px] md:text-[90px] lg:text-[176px] font-regular tracking-tight overflow-hidden max-w-full"
            >
            ...
            <div className="hidden lg:grid grid-cols-6 gap-3">
              {categories.map((category) => (
                <button
                  key={category.name}
                  onClick={() => handleCategoryClick(category.name)}
                  className={`px-4 py-3 rounded-full font-medium text-sm transition-opacity flex items-center justify-center gap-2 whitespace-nowrap ${
                    selectedCategory === category.name
                      ? "bg-white text-black hover:opacity-80"
                      : "border border-white/30 text-white hover:opacity-80"
                  }`}
                >
                  {category.name}
                </button>
