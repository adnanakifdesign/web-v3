...
function ServiceSection({ service, isLast }: { service: Service; isLast: boolean }) {
  return (
    <>
      ...
      <div className="hidden lg:grid bg-black" style={{ gridTemplateColumns: "20% 40% 40%" }}>
        <div className="p-9 md:p-7 flex flex-col justify-start" style={{ gridColumn: "1 / 2" }}>
          <div className="flex items-start gap-4 md:gap-2">
            <span className="text-[60px] font-medium tracking-tight leading-none mt-[-4px]">
              {String(service.id).padStart(2, "0")}
            </span>
            <span className="text-[16px] lg:text-[16px] uppercase tracking-tight font-medium pt-7">
              {service.title}
            </span>
          </div>
        </div>
        ...
        <div className="p-8 md:p-8 flex flex-col" style={{ gridColumn: "2 / 3" }}>
          <div className="mb-12">
            <p className="text-lg lg:text-[16px] uppercase leading-relaxed text-white/90 max-w-lg">
              {service.description}
            </p>
          </div>

          <div className="flex-1 bg-white/10 overflow-hidden aspect-[16/9]">
          ...
          <div>
            <ul className="divide-y divide-[#444]">
              {service.features.map((feature, index) => (
                <li
                  key={index}
                  className="text-sm md:text-base uppercase text-white/80 flex gap-3 leading-none items-start py-4"
                >
                  <span>{feature}</span>
                </li>
              ))}
      ...
      <div className="lg:hidden bg-black flex flex-col">
        {/* Title and Number */}
        <div className="p-8 flex flex-col justify-start">
          <div className="flex items-start gap-4">
            ...
            <span className="text-[16px] uppercase tracking-tight font-medium pt-7">{service.title}</span>
          </div>
        </div>

        {/* Description */}
        <div className="p-8 pt-0">
          <p className="text-lg uppercase leading-relaxed text-white/90">{service.description}</p>
        </div>

        {/* Image */}
        <div className="bg-white/10 overflow-hidden aspect-[16/9] mx-8">
          <img src={service.image || "/placeholder.svg"} alt={service.title} className="w-full h-full object-cover" />
        </div>

        {/* CTA Button */}
        <div className="p-8 pt-8 px-8">
      ...
      {!isLast && <div className="h-px bg-[#333] mx-8 md:mx-6 my-20" />}
    </>
  )
}
