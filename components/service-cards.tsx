  ...
  },
  {
    ...
    icon: (
      <svg className="w-30 h-30 md:w-30 md:h-30" viewBox="0 0 704.54 643.17" xmlns="http://www.w3.org/2000/svg">
        ...
        <path
          fill="#2563eb"
          d="M666.32,483.31l-11.6,4.47-83.28,32.07-15.07,43.8-15.34,44.56c-6.12,17.77-31.44,17.24-36.79-.77l-13.04-43.78-45.2-151.8c-4.69-15.75,10.18-30.32,25.83-25.31l183.45,58.64,10.02,3.2c16.66,5.32,17.34,28.65,1.02,34.93Z"
        />
      </svg>
    ),
  },
...
export function ServiceCards() {
  ...
  return (
    <div>
      <div
        ref={containerRef}
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-3 lg:gap-3 mt-16 md:mt-20 lg:mt-24"
      >
        {services.map((service, index) => (
          <div
            key={index}
            className="p-6 md:p-8 border border-[#333] bg-[#111] backdrop-blur-sm rounded-2xl"
            style={{
          ...
          >
            ...
            <h3 className="text-[30px] font-medium text-white mb-3 md:mb-4 leading-tight max-w-xs">{service.title}</h3>
            <p className="text-[16px] text-[#edeae4] leading-relaxed">{service.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 md:mt-16 lg:mt-20 flex justify-center md:hidden">
        <Link
          href="/services"
          className="w-full md:max-w-2xl px-6 md:px-8 py-4 md:py-5 bg-white text-black font-medium text-[18px] md:text-xl rounded-lg hover:bg-gray-100 transition-all duration-300 ease-out flex items-center justify-center gap-3 group"
        >
          See More Services
          <svg
            className="w-6 h-6 text-black group-hover:translate-x-1 transition-transform duration-300 ease-out"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div ref={collaborationsRef} className="mt-16 md:mt-24 lg:mt-32">
        <div className="space-y-0">
          ...
          >
            <Link
              href="/work"
              className="hidden md:flex items-center justify-between w-full max-w-lg bg-white/10 px-0 py-3 md:py-4 border-t border-gray-300 text-white font-medium text-lg md:text-xl hover:border-[#2563eb] hover:text-[#2563eb] hover:bg-[#2563eb]/20 
